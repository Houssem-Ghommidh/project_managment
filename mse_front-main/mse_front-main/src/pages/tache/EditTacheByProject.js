...
function EditTacheByProject() {
  ...
  const { idProjet, nomProjet } = location.state;
  const navigate=useNavigate();
  const {id}=useParams();

  
  async function getData(){
      const res = await axios.get(`${process.env.REACT_APP_BASE_URL}/api/tache/${id}`);
  ...
  const handleFormSubmit = async (values,{resetForm,setFieldValue}) => {
    ...
    await axios
      .put(`${process.env.REACT_APP_BASE_URL}/api/tache/${id}`, values)
      .then((response) =>
       { Swal.fire("Success", `Tache a ete mise a jour  avec succes`, "success")
    //    setTimeout(() => {
    //     window.location.href = ""
    // }, 500);
    
    isAdmin ?   navigate('/admin/cardtache',{ state: { idProjet:idProjet, nomProjet:nomProjet } }):navigate('/superviseur/cardtache',{ state: { idProjet:idProjet, nomProjet:nomProjet } })
      }
      )
      .catch((e) =>
        Swal.fire({
  ...
  const initialValues = {
    nom_tache: dataTache?.nom_tache,
    num_tache: dataTache?.num_tache,
    date_debut: dayjs(dataTache?.date_debut),
    date_fin: dayjs(dataTache?.date_fin),
    duration:dataTache?.duration,
    id_projet:idProjet
  };
  ...
  <TextField
              ...
              value={nomProjet}
              name="nomProjet"
              error={!!touched.nomProjet && !!errors.nomProjet}
              helperText={touched.nomProjet && errors.nomProjet}
...
export default EditTacheByProject
