...
:isClient?navigate("/client/formtachebyprject", { state: { idProjet: idProjet, nomProjet:nomProjet } }):navigate("/superviseur/formtachebyprject", { state: { idProjet: idProjet, nomProjet:nomProjet } });
    ...
    function goToFormulaire(e){
      ...
      :isClient?navigate("/client/cardformulaire", { state: { idProjet: idProjet, nomProjet:nomProjet,idTache:e._id,client:client } }):navigate("/superviseur/cardformulaire", { state: { idProjet: idProjet, nomProjet:nomProjet,idTache:e._id,client:client } }); 
    }
...
No Taks availble for {nomProjet}
   ...
   <Card key={i}   sx={{ backgroundColor:`${colors.primary[400]}`,gridColumn: "span 2" }}>
      ...
      <CardActions>
{   (isAdmin||isSuperviseur)&&     <Button onClick={()=>  isAdmin ? navigate(`/admin/formtachebyprject/${e._id}`,{ state: { idProjet: idProjet, nomProjet:nomProjet } }): navigate(`/superviseur/formtachebyprject/${e._id}`,{ state: { idProjet: idProjet, nomProjet:nomProjet } })} variant='contained' color='warning' size="small">Update</Button>
}    { (isAdmin||isSuperviseur)&&   <Button color='error'onClick={(tache)=> {
                Swal.fire({
                  title: 'Are you sure?',
